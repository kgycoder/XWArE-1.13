...
// Spectrum bar simulation (fake-audio using beat+noise)
const SPEC_PHASES = Array.from({length: 32}, (_, i) => Math.random() * Math.PI * 2);
...
function updateSpectrum(beat, en, h, s, l) {
  const bars = document.querySelectorAll('.np-spec-bar');
  if (!bars.length) return;

  bars.forEach((bar, i) => {
    // Simulate frequency envelope: bass heavy left, treble right
    ...
    // Target height (0-1)
    const target = Math.max(0.04,
      (0.08 + bassBoost * 0.3 + midBoost * 0.15 + noisePhase * 0.12 * en) * (1 + beatBump * en)
    );
    ...
    bar.style.boxShadow  = beat > .2 && i < 12
      ? `0 0 ${4 + beat*8}px hsla(${Math.round(h)},${s}%,70%,${(beat*.4).toFixed(2)})`
      : '';
  });
}
...
function updateNpColor() {
  ...
  if (npOpen && S.playing) {
    ...
    // 1. NP Background breathe + beat zoom
    const npBg = document.getElementById('np-bg');
    if (npBg) {
      const scBg = 1.15 + beat * .06 * en;
      const bright = .28 + beat * .08 * en;
      npBg.style.transform = `scale(${scBg.toFixed(3)})`;
      npBg.style.filter    = `blur(${82 + beat * 18 * en}px) saturate(${1.8 + beat * .6 * en}) brightness(${bright.toFixed(3)})`;
    }
    ...
    // NP not open or not playing — soft reset
    const ash = document.getElementById('np-ash');
